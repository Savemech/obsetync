...
C4Component
    ...
    Container_Boundary(b_server, "Sync Server · Rust · axum") {
        Component(app_state, "AppState", "state.rs · Arc<AppState>", "Shared state: StorageLayout, VaultStore, static X25519 key bytes, rotating in-memory transport keys, durable SequenceTracker, per-vault write locks, notify/presence state, and uptime.")
...
| **Sync API Router** | `api.rs` (handler fns) | Ten logical endpoint groups: `root`, `diff`, `content`, `manifest`, `chunk` (index), `content/chunk`, plus four `check` batch-existence endpoints. `post_diff` accepts the client's AEAD-covered root, loads both trees, calls `bridge::run_diff`, and returns metadata-complete deltas. `put_root` first proves the root belongs to the route, has a valid bounded tree shape, and references present content; only then can it fast-forward or three-way merge. Content/index GETs and checks validate Blake3 addresses; manifest GET/check parses bounded contiguous structure and verifies referenced chunks exist, so ordinary reconcile can repair damaged state. |
...
| **EphemeralKeyRotation** | `eph_rotation.rs` | Keeps current plus one previous memory-only X25519 key. `server-eph` publishes only the current public bundle; clients bootstrap against the pinned static key after restart/expiry. |
