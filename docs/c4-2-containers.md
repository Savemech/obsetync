...
| **Desktop hash pool** | 1–4 Node `worker_threads`, embedded SIMD-WASM worker bundle | Desktop-only bounded scheduler. Workers open absolute paths themselves, stream BLAKE3/FastCDC with an independent WASM instance, and return hashes/manifests plus aggregate timings. Requests and responses contain metadata only; file bytes never cross the renderer/worker boundary. M1 and x86 start at up to four workers, Windows ARM64 at two. Jobs support cancellation and reject files whose size, mtime, or inode drifts during processing. |

**Why WASM?** The Merkle tree structure and Blake3 / FastCDC algorithms need to run identically on desktop and iOS. WASM gives one Rust implementation and bounded bridge memory through adaptive 64 KiB–1 MiB feeds. Desktop workers stream source files without renderer buffers; Obsidian mobile cannot ranged-read a local source, so mobile upload still holds one capped whole-file JS buffer. Chunked downloads remain genuinely bounded.
