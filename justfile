...
ship-server: build-nix-image
    ...
    ssh {{server}} "docker load < /tmp/obsetync-nix.tar.gz && docker tag obsetync-server:nix obsetync/server:local && docker tag obsetync-server:nix ghcr.io/savemech/obsetync-nix:1.11.2 && rm /tmp/obsetync-nix.tar.gz && cd {{dest}} && docker compose up -d"
