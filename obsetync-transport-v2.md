...
### 9.3 Why HTTP 200 for the decoy, and why size is the only signal

Both response shapes — the enveloped semantic-status path and the
constant-shape decoy — leave the **HTTP** layer at `200 OK`. Size is
the only thing that distinguishes them on the wire, by design:

- 256-byte response = "your envelope wasn't openable" (decoy).
- Any other size       = "your envelope opened; the encrypted
                          plaintext carries a semantic status from
                          §9.1 that you must decrypt to read."

Why not match-status as well (i.e. return HTTP 400 on decrypt
failure)?

- A unique `400 + 256-byte` shape is itself a protocol fingerprint
  that collides operationally with every other 4xx generated by a
  reverse proxy, WAF, or `hyper` itself. Operators trying to debug
  "did my proxy block this, or did the envelope fail?" can't
  distinguish without server-side logs.
- The size already carries the same information an attacker would
  glean from a status-code split; the status code adds nothing they
  didn't have.
- HTTP 200 here doesn't require synthesizing a fake envelope — the
  body is 256 raw zero bytes, which any v0x02 decoder rejects on
  version byte (`0x00 ≠ 0x02`) before AEAD decryption even begins.

Why not pad every response to 256 bytes and eliminate the size
signal too: see §15 — metadata privacy is not a v2 goal. Operators
who need it can run the server behind Tailscale or WireGuard.
...
- `secure.rs`:
  ...
  - New helper `decoy_response()` returns the canonical 256-byte zero
    body with HTTP 200 for all decrypt failures (§9.3 explains why
    the status mirrors the success path).
- `api.rs`:
...
- Decrypt failures (each cause from §9.2) all return
  exactly-256-byte zero body with HTTP 200.
...
A `GET /api/v1/root/svx-main` round-trip with `seq = 42`, empty inner
