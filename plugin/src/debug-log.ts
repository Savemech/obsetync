...
/** Keep diagnostic lines bounded without altering their contents. Vault and
 * host paths are operational evidence and remain visible in local debug and
 * crash exports. Credentials must never enter these log messages upstream. */
...
export function sanitizeDebugLogLine(input: string): string {
    return capDebugText(String(input));
}
